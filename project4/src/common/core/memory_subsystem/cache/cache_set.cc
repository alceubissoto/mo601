...
void
CacheSet::read_line(UInt32 line_index, UInt32 offset, Byte *out_buff, UInt32 bytes, bool update_replacement, IntPtr eip)
{
   ...
   if (update_replacement) {
      ...
      updateReplacementEIP(line_index, eip);
   }
}

void
CacheSet::write_line(UInt32 line_index, UInt32 offset, Byte *in_buff, UInt32 bytes, bool update_replacement, IntPtr eip)
{
   assert(offset + bytes <= m_blocksize);
   ...
   if (m_cache_block_info_array[index]->isValid())
   {
      *eviction = true;
      ...
      if (evict_buff != NULL && m_blocks != NULL) {
         ...
         printf("Cache content (cache line): %s", evict_buff);
      }
   }
   else
   {
      ...
      UInt32 associativity, UInt32 blocksize, CacheSetInfo* set_info)
{
   ...
   switch(policy)
   {
      ...
      case CacheBase::HAWKEYE:
         return new CacheSetHAWKEYE(cache_type, associativity, blocksize);
